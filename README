...
-[courses.xml](Scheduler/courses.xml) that has the ids and the names of all the courses that must be included in the time schedule, their semester, instructor's id as well as the size of the room that is required for the course.
-[rooms.xml](Scheduler/rooms.xml) that has the ids of all the available rooms, their names as well as their size (medium, large).
-[slots.xml](Scheduler/slots.xml) that has the time slots (2*60 minutes each from Monday till Friday) for when each room is available.
